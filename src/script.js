...
/**
 * @param {{ aitalPosition: { x: number; y: number; }; }} item
 */
const updatePosition = (item) => {
  ...
  const details = document.getElementById("aital");
  if (details) {
...
const runScript = () => {
  ...
  browser.storage.sync
    .get("aitalPosition")
    .then(updatePosition)
    .catch((e) => console.error("AITAL error: could not get position from storage", e));
  ...
  browser.runtime.onMessage.addListener((message) => {
    if (message.command === "aitalUpdatePosition") {
